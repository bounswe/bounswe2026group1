...
class MediaControllerTest {
    ...
    @Test
    @DisplayName("POST /{id}/media: non-existent report returns 500 (service throws RuntimeException)")
    void upload_reportNotFound_returns500() throws Exception {
        MockMultipartFile file = new MockMultipartFile(
                "file", "photo.jpg", "image/jpeg", "bytes".getBytes());

        when(s3MediaService.uploadFile(any())).thenReturn(RETURNED_URL);
        doThrow(new RuntimeException("Report not found with id: 99"))
