      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-3">
          <ul className="flex flex-col gap-1">
            {OBJECT_TYPES.map((raw) => {
              const obj = localizeObjectType(tr, raw)
              const typeExcluded = excludedTypes.has(obj.type)
              const isOpen = expanded.has(obj.type)
              const perIssueExcluded = obj.issues.filter((i) => excludedIssues.has(`${obj.type}:${i.key}`)).length
              return (
                <li key={obj.type} className="rounded-xl bg-surface-container/40">
                  <div className="flex items-center gap-2 px-3 py-2">
                    <input
                      id={`filter-type-${obj.type}`}
                      type="checkbox"
                      checked={!typeExcluded}
                      onChange={(e) => setTypeExcluded(obj.type, !e.target.checked)}
                      className="w-4 h-4 accent-primary cursor-pointer"
                      aria-label={tr('home.showAria', { label: obj.label })}
                    />
                    <span className="material-symbols-outlined" style={{ fontSize: 20, color: obj.markerColor }}>
                      {obj.icon}
                    </span>
                    <label htmlFor={`filter-type-${obj.type}`} className="flex-1 text-sm font-semibold text-on-surface cursor-pointer">
                      {obj.label}
                      {!typeExcluded && perIssueExcluded > 0 && (
                        <span className="ml-2 text-[10px] font-normal text-on-surface-variant">
                          {tr(perIssueExcluded === 1 ? 'home.issuesHiddenOne' : 'home.issuesHiddenOther', { count: perIssueExcluded })}
                        </span>
                      )}
                    </label>
                    <button
                      type="button"
                      onClick={() => toggleExpanded(obj.type)}
                      aria-label={isOpen ? tr('home.collapseAria', { label: obj.label }) : tr('home.expandAria', { label: obj.label })}
                        ...
                        return (
                          <li key={i.key} className="flex items-center gap-2">
                            <input
                              ...
                              onChange={(e) => setIssueExcluded(obj.type, i.key, !e.target.checked)}
                              className="w-3.5 h-3.5 accent-primary cursor-pointer"
                              aria-label={tr('home.showIssueAria', { label: obj.label, issue: i.label })}
                            />
