...
function CreateReportPanel({ position, onClose, onCreated }) {
  ...
  // ── render ─────────────────────────────────────────────────────────────────
  // Mobile: 60dvh bottom sheet so the top half of the map stays clickable
  // (pin-drop hint behind us). Desktop (lg+): fixed right sidebar like the
  // view panel. The outer div uses pointer-events-none with the inner
  // <aside> opted back in via pointer-events-auto, so clicks on the
  // exposed map area pass through.
  ...
  return (
    <div className="fixed inset-0 z-[1200] pointer-events-none flex flex-col justify-end lg:flex-row lg:justify-end">
      ...
      >
        {/* Mobile sheet indicator (decorative — drag-to-resize tracked separately). */}
