...
function ProfilePage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setFormError(e.message || t('profile.profileUpdateFailed'))
    }
  }
  ...
  async function handleAvatarDelete() {
    ...
    setToast({ message: t('profile.avatarRemoved'), type: 'success' })
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto p-4 md:p-8 flex flex-col gap-6">
        {isPending && (
          <p className="text-sm text-on-surface-variant">{t('profile.loadingYours')}</p>
        )}

        {isError && (
          <p role="alert" className="text-sm text-error">
            {t('profile.loadFailed')}{error?.message ? `: ${error.message}` : '.'}
          </p>
        )}
          ...
          <>
            ...
            <section className="flex gap-3 flex-wrap">
              ...
              <StatTile
                ...
                value={user.rank != null ? `#${user.rank}` : user.leaderboardHidden ? t('profile.rankHidden') : '—'}
              />
            </section>
            ...
            <section className="bg-surface-container-lowest rounded-2xl shadow-sm p-6 flex flex-col gap-3">
              <h2 className="text-xl font-bold font-headline text-on-surface">{t('profile.leaderboard')}</h2>
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-on-surface">
                  {t('profile.hideFromLeaderboard')}
                </span>
              </label>
              <p className="text-xs text-on-surface-variant">
                {t('profile.leaderboardHideNote')}
              </p>
            </section>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <p className={`text-xs text-right ${nameInvalid ? 'text-error' : 'text-on-surface-variant'}`}>
                      {trimmedName.length < NAME_MIN
                        ? t('profile.nameMinError', { count: NAME_MIN })
